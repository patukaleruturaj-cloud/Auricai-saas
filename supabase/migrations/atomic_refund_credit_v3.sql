...
-- Filename: migrations/atomic_refund_credit_v3.sql
-- ============================================================

-- 1. Update credit_transactions schema to support 'source' and 'refund' type
ALTER TABLE public.credit_transactions 
ADD COLUMN IF NOT EXISTS source TEXT;

-- Update the type check if necessary (it already has 'refund' but let's be sure)
...
-- 2. Create the Atomic Refund RPC
...
DECLARE
    ...
    v_limit INTEGER;
BEGIN
    -- NOTE: We lock the underlying 'wallet' table row. 
    -- Locking a VIEW with JOINs (user_credit_view) is not reliably supported across Postgres versions.
